(() => {
 ...
 function finish(){clearInterval(totalTimer);btn.classList.add('hidden');cd.textContent='OPEN';document.querySelector('.door-leaf')?.classList.add('open');document.querySelector('.lock-led')?.classList.add('off');const ms=Date.now()-start;const real=fmt(ms);window.BDY?.set?.({unlock:true});out.textContent=`机械解锁完成。\n本次模拟用时：${real}\n维护培训参考：02:37（训练镜像采用压缩计时）\n事故当晚，从首次退出请求到房门开启：48:00`;document.querySelector('#unlockResult')?.classList.remove('hidden')}
